...
**AS-IS vs TO-BE 구조도** (ASCII 다이어그램):

> 현재 제품 상태와 TO-BE를 시각적으로 대비. 실제 현재 UI 기준으로 작성 (추정 금지, 스크린샷 참고).

```
 AS-IS                              TO-BE
 -----                              -----
 [현재 구조를 GNB/화면 기준으로]     [변경 후 구조]
```

### 핵심 변경 요약

> 읽는 사람이 5초 안에 "뭐가 달라지는 거구나"를 파악할 수 있는 경량 테이블. 상세 Before/After는 Implementation Spec(별도 문서)에 작성.
...
> 상세 Before/After 구현 명세: [Implementation Spec 링크] (PRD 승인 후 작성)
