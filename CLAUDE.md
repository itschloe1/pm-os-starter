# CLAUDE - PM Operating System (Starter)
...
5개의 스킬이 `.claude/skills/`에 등록되어 있다. 모든 스킬은 `context-library/`를 자동으로 참조한다.
...
`context-library/` 폴더 구조:

| 폴더/파일 | 내용 |
|-----------|------|
| `strategy/` | ICP, 비전 문서, 전략적 의사결정 |
| `prds/` | 확정된 PRD (스킬이 참조하는 레퍼런스) |
| `research/` | 유저 리서치, 경쟁사 분석 |
| `metrics/` | 핵심 지표, 대시보드 매핑 |
| `meetings/` | 주요 미팅 기록 |
| `decisions/` | 의사결정 로그 |
| `business-info.md` | 회사/제품 기본 맥락 |
| `nova-playbook.md` | Nova의 PM 기준과 리뷰 원칙 |
| `nova-prd-review-criteria.md` | PRD 리뷰 체크리스트 |

**시작할 때:** `business-info.md`부터 채우면 모든 스킬의 output 품질이 바로 올라간다.
