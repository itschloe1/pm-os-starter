...
**`/start`를 입력하세요.** MCP 연결부터 첫 결과물까지 15분이면 됩니다.
4일 온보딩 (`/day1` ~ `/day4`)을 따라가면 파워유저가 됩니다.

## Skills

25개의 스킬이 `.claude/skills/`에 등록되어 있다. 모든 스킬은 `context-library/`를 자동으로 참조한다.

**Onboarding (처음 시작할 때):**

| Skill | 설명 |
|-------|------|
| `/start` | 첫 시작. MCP 연결 → 첫 경험 → 온보딩 안내 |
| `/day1` | Day 1: 도구 연결 + /daily-plan + /meeting-notes 실습 |
| `/day2` | Day 2: /slack-message + /status-update + /action-items 실습 |
| `/day3` | Day 3: "나에 대해 알려주기" 인터뷰 → 개인 맥락 생성 → /prd-draft 개인화 |
| `/day4` | Day 4: 주간 루틴 + context-library 확장 + 커스텀 스킬 만들기 |
| `/day5` | Day 5: 피처 개발 프로세스. Discovery부터 론치까지 7단계 + Phase별 스킬 실습 |
...
| `/meeting-cleanup` | 하루 미팅 일괄 정리. 크로스미팅 인사이트, 충돌 탐지 |
...
| `/launch-comms` | 론치 커뮤니케이션 패키지 5개 동시 생성 (GTM Brief, Sales Brief, CS 브리핑, 전사 공지, 고객 이메일) |
| `/launch-checklist` | 론치 체크리스트 생성. Phase 3~5까지 마스터 트래킹 문서 |
| `/simulate-user` | ICP 페르소나로 프로토타입 UX 시뮬레이션. 실제 유저 테스트 전 1차 검증 |
...
## Recommended Workflow: 피처 개발 프로세스

피처 하나가 아이디어에서 론치까지 가는 흐름. 각 Phase에서 어떤 스킬을 쓰는지.

```
Discovery      PRD Lock       설계          개발           검증          론치          사후
(3~5일)        (~1주)         (~1주)        (~2~4주)       (~1주)        (D-day)       (+1w, +4w)

/impact-sizing /prd-draft     설계 트래킹    /create-tickets QA 확인      /launch-comms /feature-results
/simulate-user /prd-review    Open Q 해소   진행 트래킹     Dogfood       /status-update /weekly-review
               -panel                      /launch-       롤백 계획
                                            checklist
```
...
| Phase | 핵심 질문 | 스킬 |
|-------|---------|------|
| **Discovery** | "이걸 만들 가치가 있나?" | `/impact-sizing` → `/generate-ai-prototype` → `/simulate-user` |
| **PRD** | "뭘 만들고, 왜 만드는가?" | `/prd-draft` → `/prd-review-panel` |
| **설계** | "SRS+Figma가 PRD와 맞나?" | (스킬 없음, PM이 직접 트래킹) |
| **개발** | "스프린트가 잘 돌아가나?" | `/create-tickets` |
| **검증** | "론치해도 되나?" | (QA+보안+Dogfood, PM이 조율) |
| **론치** | "고객에게 어떻게 전달하나?" | `/launch-comms` (5개 동시 생성) → `/status-update` |
| **사후** | "잘 됐나? 계속할까?" | `/feature-results` → `/weekly-review` |

**Discovery에서 프로토타입 → 시뮬레이션 루프:**
1. `/impact-sizing`으로 기회 크기 확인
2. `/generate-ai-prototype`으로 프로토타입 빠르게 제작 (v0, Lovable, Bolt 등)
3. `/simulate-user`로 ICP 페르소나별 반응 시뮬레이션 (혼란 지점, 이탈 위험, 엣지 케이스)
4. 시뮬레이션 결과로 프로토타입 수정
5. 실제 고객/이해관계자에게 시연

2→3→4를 빠르게 반복한다. 실제 사람을 만나기 전에 AI로 1차 검증을 돌리는 것.
...
| `pm-process-standard.md` | PM 프로세스 표준 (Discovery→론치→사후, Phase별 체크포인트) | ✅ 포함 |
