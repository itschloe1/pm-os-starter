...
23개의 스킬이 `.claude/skills/`에 등록되어 있다. 모든 스킬은 `context-library/`를 자동으로 참조한다.
...
/impact-sizing /prd-draft     설계 트래킹    /create-tickets QA 확인      /sales-brief  /feature-results
/simulate-user /prd-review    Open Q 해소   진행 트래킹     Dogfood       /slack-message /weekly-review
               -panel                      블로커 해소     롤백 계획     /status-update
```
...
| **론치** | "고객에게 어떻게 전달하나?" | `/sales-brief` → `/slack-message` → `/status-update` |
