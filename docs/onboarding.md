...
> **처음이라면 `/start`를 입력하세요.** MCP 연결부터 첫 결과물까지 15분이면 됩니다.
> 그 다음 `/day1` ~ `/day4`를 하루에 하나씩 따라가면 4일 만에 파워유저가 됩니다.
>
> 아래 내용은 레퍼런스 문서입니다. `/start`로 시작하는 걸 권장합니다.

---

> 이 워크스페이스를 처음 쓰는 PM을 위한 가이드.
...
| `/launch-comms` | 론치 커뮤니케이션 패키지 (5개 동시) | 론치 1주 전, PRD 넣으면 GTM Brief/Sales Brief/CS 브리핑/전사 공지/고객 이메일 동시 생성 |
| `/launch-checklist` | 론치 체크리스트 | 개발 Phase에서 시작, 론치까지 마스터 트래킹 |
| `/sales-brief` | 세일즈 브리프 | 제품 변경이 고객에게 영향줄 때 (독립 실행도 가능) |
...
### 릴리즈 전
1. `/launch-checklist` → 론치 체크리스트 초안 (개발 중 시작)
2. `/launch-comms` → 론치 1주 전, 커뮤니케이션 패키지 5개 동시 생성
