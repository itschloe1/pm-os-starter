...
6. `memory/temp/` - Pending actions and short-term items (cleanup during this skill)
...
### Step 2.5: Memory Temp Cleanup

Scan `memory/temp/` files and clean up volatile items. See `memory/temp/README.md` for rules.

**For each item in temp/ files:**

| Status | Action |
|--------|--------|
| Completed action item | Delete from file |
| Resolved decision | If strategically important, move to `memory/strategic-decisions.md`. Otherwise delete. |
| 1+ week old, no change | Ask Nova: "이 항목 아직 유효해?" and act on answer |
| Still active and relevant | Keep, update status if needed |

**After cleanup:**
- Update `Next review:` date in each temp file to next week
- If a temp file becomes empty, delete it
- Briefly report: "Temp 정리: [X]건 완료 삭제, [Y]건 유지, [Z]건 승격"
