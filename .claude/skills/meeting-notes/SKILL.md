...
After processing meeting notes, if the "Decisions Made" section contains any entries, automatically create a Lightweight Decision Doc for each decision.

### Process

1. **Detect:** Check if "Decisions Made" section has one or more entries after processing
2. **Transform:** Convert each decision into a Lightweight Decision Doc
3. **Save:** Write to `outputs/decisions/[YYYY-MM-DD]-decision-[kebab-case-topic].md`
4. **Link:** Add `→ Decision doc: outputs/decisions/...` reference in the meeting notes after each captured decision
5. **Suggest:** Include note: "이 결정을 더 깊이 분석하려면 `/decision-doc`을 실행하세요"

### Lightweight Decision Doc Format
...
**Date:** [Meeting Date]
**Owner:** [Who decided]
**Source:** [Link to meeting notes file]

## The Decision
[Decision statement from meeting notes]

## Rationale
[Why, extracted from meeting notes Decisions Made section]

## Impact
[Impact from meeting notes Decisions Made section]

## Next Steps
- [Related action items from meeting notes, if any]

## How We'll Know It Worked
[If mentioned in meeting; otherwise "TBD - define success metric"]
```

### Rules
- Only capture decisions that are **final** (not "to be discussed further" or "pending approval")
- If multiple decisions in one meeting, create separate files for each
- If a decision references a previous decision in `outputs/decisions/` or `context-library/decisions/`, link to it
- Keep the doc lightweight. If the PM wants deeper analysis, they can run `/decision-doc`
...
- [ ] **Decisions auto-captured:** If Decisions Made section has entries, corresponding files exist in `outputs/decisions/`
