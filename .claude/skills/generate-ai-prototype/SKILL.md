...
description: Generate v0.dev, Lovable, or Bolt.new prompts for AI-powered prototyping from PRDs or feature specs
...
Read relevant files:
- PRD or feature spec from `context-library/prds/` or `outputs/prds/`
...
- Too many features in first prompt (overwhelming). Core flow first, add features incrementally.
- Vague descriptions ("nice interface"). Use specific details ("Card layout, 3 columns, shadcn components").
- No example data (generic placeholders). Use realistic sample data (actual user names, realistic numbers).
- Forgetting edge cases (only happy path). Include empty, loading, error states.
- No iteration plan (one-and-done). Expect 2-3 rounds of refinement.
